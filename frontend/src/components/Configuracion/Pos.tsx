...
const posService = {
  ...
  getAllPos: async (): Promise<Pos[]> => {
    try {
      console.log('Obteniendo dispositivos POS desde:', `${API_BASE_URL}/pos`);
      const response = await axios.get(`${API_BASE_URL}/pos`);
  ...
  getPosByCodigoBarras: async (codigo: string): Promise<Pos | null> => {
    try {
      ...
      console.log('URL:', `${API_BASE_URL}/pos/codigo/${codigo}`);
      
      const response = await axios.get(`${API_BASE_URL}/pos/codigo/${codigo}`);
      ...
      if (error.response && error.response.status === 404) {
        // Si no se encuentra, retornamos null en lugar de lanzar error
        return null;
      }
      if (error.response) {
        console.error('Respuesta del servidor:', error.response.data);
  ...
  createPos: async (pos: Omit<Pos, 'id'>): Promise<Pos> => {
    try {
      ...
      console.log('URL:', `${API_BASE_URL}/pos`);
      
      const response = await axios.post(`${API_BASE_URL}/pos`, pos);
  ...
  updatePos: async (id: number, pos: Partial<Pos>): Promise<Pos> => {
    try {
      ...
      console.log('URL:', `${API_BASE_URL}/pos/${id}`);
      
      const response = await axios.put(`${API_BASE_URL}/pos/${id}`, pos);
  ...
  deletePos: async (id: number): Promise<void> => {
    try {
      ...
      await axios.delete(`${API_BASE_URL}/pos/${id}`);
...
const cuentaBancariaService = {
  ...
  getAllCuentasBancarias: async (): Promise<CuentaBancaria[]> => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/cuentas-bancarias`);
              ...
              <Grid item xs={12}>
                <FormControl fullWidth required>
                  ...
                  <Select
                    ...
                    value={formData.cuentaBancariaId.toString()}
