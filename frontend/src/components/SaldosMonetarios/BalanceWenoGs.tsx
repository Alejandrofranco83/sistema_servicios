...
const BalanceWenoGs: React.FC = () => {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.response?.data?.error || 'Error al cargar los datos de Wepa Gs');
            ...
            <Grid item xs={12} md={7}>
              <Card sx={{ height: '100%', bgcolor: '#1E1E1E', color: 'white' }}>
                <CardHeader 
                  ...
                  subheader="Movimientos de Wepa Gs por caja"
                  titleTypographyProps={{ variant: 'h6' }}
                  subheaderTypographyProps={{ color: 'rgba(255, 255, 255, 0.7)' }}
                />
                <Divider sx={{ borderColor: 'rgba(255, 255, 255, 0.12)' }} />
